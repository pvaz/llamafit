...
- The Downloads screen in the TUI and the install endpoints in the API — **not built**. These
  four are the command line only; `llamafit` with no arguments and `llamafit serve` still read
...
- A paging detector for the silent NVIDIA driver fallback (VRAM pinned at the ceiling plus a
  speed drop) — done, in three states: paging, not paging, and unchecked.

The commands ship. What the phase still owes is its own proof: this phase is done when
estimates on the reference machine are within 15 percent after calibration, and they are not,
because the calibration a run produces is not yet applied to anything.
