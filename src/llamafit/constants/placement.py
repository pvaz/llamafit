...
"""Constants for the placement planner (design section 9).

Where a model's bytes go, and the settings that put them there -- the run modes and
ladders the search walks, the context ladder a launch script chooses from at start time,
and the ``llama-server`` command line the search renders (section 9.5).
"""
