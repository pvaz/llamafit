...
- `sources[].path` is optional for a `gguf` source and names a directory inside the repository; only files under it are matched against that source's quants and extras. It exists because a repository routinely publishes the same quant name twice, a plain build and an importance-matrix build in two directories, and the matcher refuses to guess between them. Unset, every file in the repository is considered, so no existing entry changes meaning. A `path` that holds no files is a curated mistake and leaves the model untouched.
