...
arrived; an incomplete set is an error, because facts derived from it are silently too small rather
than visibly absent.
...
The exclusions are in three places and a reader should be told where: **11.1** excludes a model lacking a required capability; **11.2** excludes one that cannot hold `--min-context`; **11.4** excludes one slower than its reader. All three are exclusions rather than filters, so the candidate appears with its reason, its placement and whatever else was learned about it, and all three name the thing the reader can change.
