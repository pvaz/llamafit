...
Every model family is two files: a curated `<family>.yaml` and a generated
...
merges it onto the curated data every time the catalog is read, so a model's
`sources[].quants[]` look fully populated in code even though the YAML on disk only ever
names the quant:
...
written atomically, to a sibling temporary file and then moved into place, so a crash or a
full disk never leaves a truncated one. A family with no facts file yet has simply never been
refreshed.
...
- **A model whose sources reuse a quant name receives no facts at all.** The file is keyed by
  ...
  the way out, because the name is what files are matched against. Keep one source for that name, or
...
generation that numbers it, so Llama 3.1, 3.2 and 3.3 are all `llama3` and Granite 4.2 is
...
ship a very large lookup table that llama.cpp streams from disk rather than holding in memory,
such as Qwen3.8-Flash-Next's `per_layer_token_embd.weight` at 28.8 GB, a quarter of its
download. Nothing in the header marks it as such, and
...
percentage from 0 to 100. A vendor who publishes a fraction, as Mistral does when it prints
Arena Hard as `0.305`, is converted to that scale, because a column mixing `0.86` and `86.0` cannot be
...
`MATH (maj@1)` rather than `MATH`, since two harnesses rarely produce the same number.
...
several requests, and why a first shard that carries metadata and no tensors, which is a
normal layout, does not silently produce a model whose every byte figure is zero.
...
routinely document less context than the file permits, and Qwen3-0.6B is documented at 32,768
tokens where its GGUF header declares 40,960, so a check that flagged every conservative
...
across the catalog by design, since a measurement is a real run on real hardware and there
have not been many, but it is thinnest exactly where a first-time reader is most likely to
be standing. One entry in the whole catalog carries a CPU-only run: `qwen3-0.6b`, at `-ngl 0
